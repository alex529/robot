...
void state_approach_corner_logic() {
	...
	if (tmr_exp(&state_approach_corner_timer)){
		set_m_backward()
		l_motor.rpm = 0;
		r_motor.rpm = 0;
...
void state_go_ahead_after_turn_logic() {
		...
		if (tmr_exp(&state_go_ahead_after_turn_timer)){
...
void state_third_go_ahead_logic() {
	if (state_third_go_ahead_data.not_first_run == false){
		...
		if (tmr_exp(&state_third_go_ahead_timer)){
			set_m_backward()
			r_motor.rpm = 0;
			l_motor.rpm = 0;
...
void state_last_go_ahead_logic() {
	if (state_last_go_ahead_data.not_first_run == false){
		...
		if (tmr_exp(&state_last_go_ahead_timer)){
			set_m_backward()
			r_motor.rpm = 0;
			l_motor.rpm = 0;
